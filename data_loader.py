...
class Dataset(data.Dataset):
    def _preprocess(self):
        ...
        if not self.training and last_bag is not None:
            self.data.append(bag)
    ...
    def __getitem__(self, index):
        ...
        rel = torch.tensor(item['rel'],  dtype=torch.long)
        if not self.training:
            rel_mul = torch.zeros(len(self.rel2id), dtype=torch.long)
            for i in set(rel):
                rel_mul[i] = 1
            rel = rel_mul
        return word, pos1, pos2, mask, length, rel
...
def collate_fn(X):
    X = list(zip(*X))
    word, pos1, pos2, mask, length, rel = X
    scope = []
    ind = 0
    for w in word:
    ...
    rel = torch.stack(rel)
    return word, pos1, pos2, mask, length, scope, rel
